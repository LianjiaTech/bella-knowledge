...
public class FileControllerBatchCountTest {
    ...
    public void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(controller).setControllerAdvice(new FileApiResponseAdvice())
    ...
    public void batchCount_withValidRequest_returnsCountList() throws Exception {
        ...
        List<FileCountInfo> expectedResult = Arrays.asList(new FileCountInfo("dir-1", 5), new FileCountInfo("dir-2", 3),
        ...
        mockMvc.perform(post("/v1/files/batch-count").contentType(MediaType.APPLICATION_JSON).content(requestBody)).andExpect(status().isOk())
            .andExpect(jsonPath("$[0].ancestor_id").value("dir-1")).andExpect(jsonPath("$[0].count").value(5))
            .andExpect(jsonPath("$[1].ancestor_id").value("dir-2")).andExpect(jsonPath("$[1].count").value(3))
            .andExpect(jsonPath("$[2].ancestor_id").value("dir-3")).andExpect(jsonPath("$[2].count").value(8));
    ...
    public void batchCount_withNullType_returnsCountList() throws Exception {
        ...
        List<FileCountInfo> expectedResult = Arrays.asList(new FileCountInfo("dir-1", 12), new FileCountInfo("dir-2", 7));
        ...
        mockMvc.perform(post("/v1/files/batch-count").contentType(MediaType.APPLICATION_JSON).content(requestBody)).andExpect(status().isOk())
            .andExpect(jsonPath("$[0].ancestor_id").value("dir-1")).andExpect(jsonPath("$[0].count").value(12))
            .andExpect(jsonPath("$[1].ancestor_id").value("dir-2")).andExpect(jsonPath("$[1].count").value(7));
    ...
    public void batchCount_withNullRequest_returns400() throws Exception {
        mockMvc.perform(post("/v1/files/batch-count").contentType(MediaType.APPLICATION_JSON).content("{}")).andExpect(status().isBadRequest())
    ...
    public void batchCount_withEmptyAncestorIds_returns400() throws Exception {
        ...
        mockMvc.perform(post("/v1/files/batch-count").contentType(MediaType.APPLICATION_JSON).content(requestBody)).andExpect(status().isBadRequest())
            .andExpect(jsonPath("$.error.message").value(org.hamcrest.Matchers.containsString("ancestor_ids is required and cannot be empty")));
    }
